...
TiltFiveSpectatorController::TiltFiveSpectatorController(FTiltFiveXRBase* InHMDDevice)
	: HMDDevice(InHMDDevice)
{
}


ESpectatorScreenMode TiltFiveSpectatorController::GetSpectatorScreenMode() const
{
...
void TiltFiveSpectatorController::SetSpectatorScreenMode(ESpectatorScreenMode Mode)
{
	FScopeLock FrameLock(&NewSpectatorScreenModeLock);
	NewSpectatorScreenMode = Mode;
}

void TiltFiveSpectatorController::SetSpectatorScreenTexture(UTexture* SrcTexture)
{
	SpectatorScreenTexture = SrcTexture;
}

UTexture* TiltFiveSpectatorController::GetSpectatorScreenTexture() const
{
...
void TiltFiveSpectatorController::SetSpectatorScreenModeTexturePlusEyeLayout(const FSpectatorScreenModeTexturePlusEyeLayout& Layout)
{
	if (Layout.IsValid())
	{
...
void TiltFiveSpectatorController::QueueDebugCanvasLayerID(int32 LayerID)
{
	DebugCanvasLayerIDs.Add(LayerID);
}

FSpectatorScreenRenderDelegate* TiltFiveSpectatorController::GetSpectatorScreenRenderDelegate_RenderThread()
{
...
{
	TiltFiveSpectatorController* SpectatorScreenController;
	FTexture2DRHIRef Texture;

	FORCEINLINE_DEBUGGABLE FRHISetSpectatorScreenTexture(TiltFiveSpectatorController * InSpectatorScreenController, const FTexture2DRHIRef & InTexture)
...
void TiltFiveSpectatorController::SetSpectatorScreenTextureRenderCommand(UTexture* SrcTexture)
{
	...
	TiltFiveSpectatorController* SpectatorScreenController = this;
...
void TiltFiveSpectatorController::SetSpectatorScreenTexture_RenderThread(FTexture2DRHIRef& InTexture)
{
...
FRHICOMMAND_MACRO(FRHISetSpectatorScreenModeTexturePlusEyeLayout)
{
	...
	FORCEINLINE_DEBUGGABLE FRHISetSpectatorScreenModeTexturePlusEyeLayout(TiltFiveSpectatorController * InSpectatorScreenController, const FSpectatorScreenModeTexturePlusEyeLayout & InLayout)
		: SpectatorScreenController(InSpectatorScreenController)
		, Layout(InLayout)
	{
	}
...
void TiltFiveSpectatorController::SetSpectatorScreenModeTexturePlusEyeLayoutRenderCommand(const FSpectatorScreenModeTexturePlusEyeLayout& NewLayout)
{
	check(IsInGameThread());
...
void TiltFiveSpectatorController::SetSpectatorScreenModeTexturePlusEyeLayout_RenderThread(const FSpectatorScreenModeTexturePlusEyeLayout& Layout)
{
	SpectatorScreenModeTexturePlusEyeLayout_RenderThread = Layout;
}

void TiltFiveSpectatorController::BeginRenderViewFamily()
{
	check(IsInGameThread());
...
void TiltFiveSpectatorController::UpdateSpectatorScreenMode_RenderThread()
{
	...
	case ESpectatorScreenMode::SingleEyeLetterboxed:
		RenderDelegate->BindRaw(this, &TiltFiveSpectatorController::RenderSpectatorModeSingleEyeLetterboxed);
		break;
	case ESpectatorScreenMode::Undistorted:
		RenderDelegate->BindRaw(this, &TiltFiveSpectatorController::RenderSpectatorModeUndistorted);
		break;
	case ESpectatorScreenMode::Distorted:
		RenderDelegate->BindRaw(this, &TiltFiveSpectatorController::RenderSpectatorModeDistorted);
		break;
	case ESpectatorScreenMode::SingleEye:
		RenderDelegate->BindRaw(this, &TiltFiveSpectatorController::RenderSpectatorModeSingleEye);
		break;
	case ESpectatorScreenMode::Texture:
		RenderDelegate->BindRaw(this, &TiltFiveSpectatorController::RenderSpectatorModeTexture);
		break;
	case ESpectatorScreenMode::TexturePlusEye:
		RenderDelegate->BindRaw(this, &TiltFiveSpectatorController::RenderSpectatorModeMirrorAndTexture);
		break;
	default:
		RenderDelegate->BindRaw(this, &TiltFiveSpectatorController::RenderSpectatorModeSingleEyeCroppedToFill);
		break;
	}
}

void TiltFiveSpectatorController::RenderSpectatorScreen_RenderThread(FRHICommandListImmediate& RHICmdList, FRHITexture2D* BackBuffer, FTexture2DRHIRef SrcTexture, FVector2D WindowSize)
{
	RenderSpectatorScreen_RenderThread(RHICmdList, BackBuffer, SrcTexture, nullptr, WindowSize);
}

void TiltFiveSpectatorController::RenderSpectatorScreen_RenderThread(FRHICommandListImmediate& RHICmdList, FRHITexture2D* BackBuffer, FTexture2DRHIRef SrcTexture, FTexture2DRHIRef LayersTexture, FVector2D WindowSize)
{
	...
	if (StereoLayers && !LayersTexture)
	{
		...
		for (int32 LayerID : DebugCanvasLayerIDs)
		{
			...
			if (LayerTexture)
			{
				...
				const FIntRect DstRectLetterboxed = GetLetterboxedDestRect(LayerRect, DstRect);
...
FIntRect TiltFiveSpectatorController::GetFullFlatEyeRect_RenderThread(FTexture2DRHIRef EyeTexture)
{
	return HMDDevice->GetFullFlatEyeRect_RenderThread(EyeTexture);
}

void TiltFiveSpectatorController::CopyEmulatedLayers(FRHICommandListImmediate& RHICmdList, FTexture2DRHIRef TargetTexture, const FIntRect SrcRect, const FIntRect DstRect)
{
...
void TiltFiveSpectatorController::RenderSpectatorModeUndistorted(FRHICommandListImmediate& RHICmdList, FTexture2DRHIRef TargetTexture, FTexture2DRHIRef EyeTexture, FTexture2DRHIRef OtherTexture, FVector2D WindowSize)
{
...
void TiltFiveSpectatorController::RenderSpectatorModeDistorted(FRHICommandListImmediate& RHICmdList, FTexture2DRHIRef TargetTexture, FTexture2DRHIRef EyeTexture, FTexture2DRHIRef OtherTexture, FVector2D WindowSize)
{
...
void TiltFiveSpectatorController::RenderSpectatorModeSingleEye(FRHICommandListImmediate& RHICmdList, FTexture2DRHIRef TargetTexture, FTexture2DRHIRef EyeTexture, FTexture2DRHIRef OtherTexture, FVector2D WindowSize)
{
...
void TiltFiveSpectatorController::RenderSpectatorModeSingleEyeLetterboxed(FRHICommandListImmediate& RHICmdList, FTexture2DRHIRef TargetTexture, FTexture2DRHIRef EyeTexture, FTexture2DRHIRef OtherTexture, FVector2D WindowSize)
{
	const FIntRect SrcRect = GetFullFlatEyeRect_RenderThread(EyeTexture);
	const FIntRect DstRect(0, 0, TargetTexture->GetSizeX(), TargetTexture->GetSizeY());
	const FIntRect DstRectLetterboxed = GetLetterboxedDestRect(SrcRect, DstRect);
...
void TiltFiveSpectatorController::RenderSpectatorModeSingleEyeCroppedToFill(FRHICommandListImmediate& RHICmdList, FTexture2DRHIRef TargetTexture, FTexture2DRHIRef EyeTexture, FTexture2DRHIRef OtherTexture, FVector2D WindowSize)
{
	const FIntRect SrcRect = GetFullFlatEyeRect_RenderThread(EyeTexture);
	const FIntRect DstRect(0, 0, TargetTexture->GetSizeX(), TargetTexture->GetSizeY());
	const FIntRect WindowRect(0, 0, static_cast<int32>(WindowSize.X), static_cast<int32>(WindowSize.Y));

	const FIntRect SrcCroppedToFitRect = GetEyeCroppedToFitRect(HMDDevice->GetEyeCenterPoint_RenderThread(EStereoscopicEye::eSSE_LEFT_EYE), SrcRect, WindowRect);
...
void TiltFiveSpectatorController::RenderSpectatorModeTexture(FRHICommandListImmediate& RHICmdList, FTexture2DRHIRef TargetTexture, FTexture2DRHIRef EyeTexture, FTexture2DRHIRef OtherTexture, FVector2D WindowSize)
{
...
void TiltFiveSpectatorController::RenderSpectatorModeMirrorAndTexture(FRHICommandListImmediate& RHICmdList, FTexture2DRHIRef TargetTexture, FTexture2DRHIRef EyeTexture, FTexture2DRHIRef OtherTexture, FVector2D WindowSize)
{
	...
	const FIntRect CroppedEyeSrcRect = GetEyeCroppedToFitRect(HMDDevice->GetEyeCenterPoint_RenderThread(EStereoscopicEye::eSSE_LEFT_EYE), EyeSrcRect, EyeDstRect);
...
FRHITexture2D* TiltFiveSpectatorController::GetFallbackRHITexture() const
{
...
FIntRect TiltFiveSpectatorController::GetEyeCroppedToFitRect(FVector2D EyeCenterPoint, const FIntRect& SrcRect, const FIntRect& TargetRect)
{
...
FIntRect TiltFiveSpectatorController::GetLetterboxedDestRect(const FIntRect& SrcRect, const FIntRect& TargetRect)
{
